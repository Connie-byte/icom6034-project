                      ...
                      .then((result) => {
                          if (result && result.lat && result.lng) {
                              ...
                              marker.addListener('click', function() {
                                  // Retrieve the destination information from the marker's title
                                  var destination = this.getTitle();

                                  // Send an AJAX request to a controller method that retrieves the related information from the database
                                  $.ajax({
                                      type: 'GET',
                                      url: '/destination/show',
                                      data: { 'destination': destination },
                                      success: function(response) {
                                          // Redirect the user to the destination view with the related information
                                          window.location.href = '/destination/' + response.id;
                                      },
